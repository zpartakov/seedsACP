...
function addToCart()
{
	...
	if (dbNumRows($result) == 0) {
		...
		$sql = "INSERT INTO plantons_cart (pd_id, ct_qty, ct_session_id, ct_date)
				VALUES ($productId, 1, '$sid', NOW())";
		$result = dbQuery($sql);
	} else {
		// update product quantity in cart table
		$sql = "UPDATE plantons_cart 
		        SET ct_qty = ct_qty + 1
...
function getCartContent()
{
	...
	$sql = "SELECT ct_id, ct.pd_id, ct_qty, pd_name, pd_price, pd_thumbnail, pd.cat_id
			FROM plantons_cart ct, plantons_product pd, plantons_category cat
